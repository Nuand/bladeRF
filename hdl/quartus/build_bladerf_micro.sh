#!/usr/bin/env bash
#
# Build a bladeRF ADI FPGA image
################################################################################

function usage()
{
    echo ""
    echo "bladeRF-ADI FPGA build script"
    echo ""
...
if [ "$size" -ne 25 ] && [ "$size" -ne 49 ] && [ "$size" -ne 301 ]; then
...
nios_system=../fpga/ip/altera/nios_system_micro

device="5CEB"
if [ "$size" -eq 25 ]; then
    device="${device}A2"
elif [ "$size" -eq 49 ]; then
    device="${device}A4"
elif [ "$size" -eq 301 ]; then
    device="${device}A9"
fi
device="${device}F23C8"
...
mkdir -p work
pushd work
$quartus_sh -t ../bladerf_micro.tcl
if [ "$stp" == "" ]; then
    $quartus_sh -t ../build_micro.tcl -rev $rev -size $size
else
    $quartus_sh -t ../build_micro.tcl -rev $rev -size $size -stp $stp $force
fi
popd
