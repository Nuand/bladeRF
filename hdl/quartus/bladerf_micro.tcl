...
proc make_revision { rev } {
    if [revision_exists $rev] {
        project_open -force -revision $rev bladerf
    } else {
        create_revision -based_on base -set_current $rev
    }
    set_global_assignment -name QIP_FILE [file normalize ../../fpga/platforms/bladerf-micro/bladerf-$rev.qip]
    export_assignments
}

# Create the base project
if [project_exists bladerf] {
    project_open -force -revision base bladerf
} else {
    project_new -family "Cyclone V" -revision base bladerf
}
...
set_global_assignment -name DEVICE_MIGRATION_LIST "5CEBA2F23C8,5CEBA4F23C8,5CEBA9F23C8"
...
set_global_assignment -name QIP_FILE [file normalize ../../fpga/platforms/bladerf-micro/bladerf.qip]
source [file normalize ../../fpga/platforms/bladerf-micro/constraints/pins.tcl]
