...
static int test_format_sizes(void)
{
    ...
    if (samples_to_bytes(BLADERF_FORMAT_SC16_Q11_PACKED, num_samples) !=
        ...
        samples_to_wire_bytes(BLADERF_FORMAT_SC16_Q11_PACKED, num_samples) !=
            3 * num_samples ||
        wire_bytes_to_samples(BLADERF_FORMAT_SC16_Q11_PACKED,
                              3 * num_samples) != num_samples) {
...
{
    size_t const transfer_quantum = 8;

    if (wire_buffer_size_is_valid(BLADERF_FORMAT_SC16_Q11_PACKED, 7,
                                  transfer_quantum) ||
        !wire_buffer_size_is_valid(BLADERF_FORMAT_SC16_Q11_PACKED, 8,
                                   transfer_quantum) ||
        !wire_buffer_size_is_valid(BLADERF_FORMAT_SC16_Q11_PACKED, 16,
                                   transfer_quantum)) {
        fprintf(stderr, "packed wire buffer validation failed\n");
        return 1;
    }
...
int main(void)
{
    return test_format_sizes() + test_buffer_sizes() +
