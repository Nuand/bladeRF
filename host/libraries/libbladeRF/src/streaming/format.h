...
/* Convert wire bytes to samples based upon the provided format */
static inline size_t wire_bytes_to_samples(bladerf_format format, size_t n)
{
    switch (format) {
        case BLADERF_FORMAT_SC8_Q7:
        case BLADERF_FORMAT_SC8_Q7_META:
...
static inline bool wire_buffer_size_is_valid(bladerf_format format,
                                             size_t num_samples,
                                             size_t transfer_quantum)
{
    size_t const wire_size = samples_to_wire_bytes(format, num_samples);

    return transfer_quantum != 0 && wire_size >= transfer_quantum &&
           wire_size % transfer_quantum == 0;
}
