 ...
 * LMS internal DC offset auto-calibrations
     ...
     * `calibrate lms rxvga2 [<DC ref> <stage 1 I> <stage 1 Q> <stage 2 I> <stage 2 Q>]`
 ...
 * RX and TX I/Q DC offset correction parameter calibration
    ...
    Calibrate the DC offset correction paremeters for the current frequency and gain settings. If a I/Q values are provided, they are applied directly. '`cal rxtx`' is shorthand for '`cal rx`' followed by '`cal tx`'.
...
If `num_addresses` is supplied, the address is incremented by 1 and another peek is performed.
...
If `num_addresses` is supplied, the address is incremented by 1 and another poke is performed.
...
The `wait` command takes an optional timeout parameter. This parameter defaults to units of milliseconds (`ms`). The timeout unit may be specified using the `ms`, `s`, `m`, or `h` suffixes. If this parameter is not provided, the command will wait until the reception completes or `Ctrl-C` is pressed.
 ...
 * `timeout` - Data stream timeout. With no suffix, the default unit is `ms`. The default value is 1s. Valid suffixes are '`ms`' and '`s`'.
...
The wait command takes an optional timeout parameter. This parameter defaults to units of milliseconds (`ms`). The timeout unit may be specified using the `ms`, `s`, `m`, or `h` suffixes. If this parameter is not provided, the command will wait until the transmission completes or `Ctrl-C` is pressed.
 ...
 * The `n`, `samples`, `buffers`, and `xfers` parameters support the suffixes '`K`', '`M`', and '`G`', which are multiples of 1024.
 * For higher sample rates, it is advised that the input file be stored in RAM (e.g. `/tmp`, `/dev/shm`) or to an SSD, rather than a HDD.
